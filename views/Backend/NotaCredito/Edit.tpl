...
<h1>{if $data.Id}Editar{else}Nueva{/if} Nota de credito &raquo; <span>{$data.Numero} (Interno {$data.Id})</span></h1>
...
<form id="FrmEdit" enctype="multipart/form-data" action="" method="post">
   <input type="hidden" name="data[Id]" value="{$data.Id}" />
    ...
    <div class="form">
        {include file="Backend/NotaCredito/ColumnForm.tpl"}
        ...
        <div class="contentEditor"> <!-- formulario de edicion de contenido -->
            ...
            <div class="productsEditorContent">
                		...
                		<div class="buttonsCont" style="clear: left;">
							{if $data.Id}
							<input type="" value="Imprimir" href="/NotaCredito/NotaCreditoView/id/{$data.Id}" class="btDark printCredito" title="Imprimir nota de credito" />
							{/if}
                ...
                <div class="contentTitles">
                    ...
                    <textarea name="data[Descripcion]" cols="45" rows="5" class="required" id="Descripcion" {if $data.Id}readonly="readonly" disabled{/if}>{$data.Descripcion}</textarea>
                </div> <!-- /contentTitles -->
