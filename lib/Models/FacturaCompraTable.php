...
class FacturaCompraTable extends Doctrine_Table
{
	 ...
	 */
	public function GetPendientesDePagoDistintasDeInsumosContabilizar($filters)
	{
		// obtener IDs de las facturas no paga y agregar where in
		//$OrdenesDePagoIds	=	Doctrine_Query::create()
		//						->from('OrdenDePago');
		...
		$q->andWhere('(f.TipoGastoId <> ?) OR (f.TipoGastoId IS NULL)', $CONST_InsumosAContabilizarId);
		//echo $q->getSqlQuery();
		$q->orderBy('f.FechaVencimiento DESC');
