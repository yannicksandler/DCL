<?php
    ...
    {   
        ...
        public function GetProveedoresDistintosDeInsumosContabilizar()
        {
        	$clientes	=	array();
        	...
        	//->innerJoin('c.FacturasCompra f')
        	//->andWhere('f.TipoGastoId <> ?', $CONST_InsumosAContabilizarId)
        	->andWhere('c.TipoGastoId <> ? OR c.TipoGastoId IS NULL', $CONST_InsumosAContabilizarId)
        	//->andWhere('c.SaldoActual < 0')
        	->groupBy('c.Id')
        	->orderBy('c.SaldoActual DESC');
        	//echo $q->getSqlQuery();
        ...
        /*
         * implemented: 12/08/2014
         * updated: 20-8-2014: mostrar proveedores con tipo de gasto null
         */
        public function GetSaldoTotalProveedoresDistintosDeInsumosContabilizar()
        {
        	// No mostrar Insumos a contabilizar, exite otro procedimiento para tratarlas
        	// Insumos a contabilizar
        	$CONST_InsumosAContabilizarId = 5;
...
if(!is_numeric($Cheque->Cuenta))
        		throw new Exception('El cheque no tiene una cuenta bancaria asignada');
