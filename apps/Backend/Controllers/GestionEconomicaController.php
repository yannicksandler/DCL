...
class Backend_Controllers_GestionEconomicaController extends IDS_Controller_Action_Standard {
	...
	public function MainAction()
	{
		...
		if($a->GetUser()->Id)
		{
			...
			if(is_object($usuario))
			{
				...
				$view->SetParam("SaldoTotalProveedores", $g->GetSaldoTotalProveedoresDistintosDeInsumosContabilizar());
				/*
	...
	public function SaldosProveedoresAction()
	{
		...
		if($a->IsLoggedIn())
		{
			...
			$filters	=	array();
			$g	=	new Classes_GestionEconomicaManager();
			$view->SetParam("Proveedores", $g->GetProveedoresDistintosDeInsumosContabilizar());
			$view->SetParam("SaldoTotalProveedores", $g->GetSaldoTotalProveedoresDistintosDeInsumosContabilizar());
		}
	}
	...
	public function AcreditarChequeAction()
	{
		try {
			...
			if($a->IsLoggedIn())
			{
				...
				if(!is_numeric($ChequeId) and !($ChequeId > 0))
				{
